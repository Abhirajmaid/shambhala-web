...
export function ProjectDetail({ projectId }) {
  ...
  const [form, setForm] = useState({ name: '', category: '', tagline: '', status: 'Draft' });
        ...
        <form onSubmit={saveDetails} className="rounded-3xl bg-white/90 p-6 shadow-[0_14px_35px_rgba(15,23,42,0.06)] backdrop-blur">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 pt-2">
              {editing && (
                <>
                  <Button disabled={saving} className="bg-[#8a5a32] text-white hover:bg-[#704626]">{saving ? 'Saving...' : 'Save details'}</Button>
            ...
            {displayImages.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {visibleImages.map((image, index) => (
                  <div key={`${image.url}-${index}`} className="group relative h-44 overflow-hidden rounded-2xl bg-slate-100">
                    <img src={image.url} alt={`${project.name} image ${index + 1}`} className="h-full w-full object-cover" />
                    <button
                      type="button"
